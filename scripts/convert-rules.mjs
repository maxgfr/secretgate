...
export async function convertConfig() {
  ...
  for (const raw of toml.rules ?? []) {
    const id = raw.id;
    // Path-only rules (no content regex) flag FILES by name, not content —
    // they go to PATH_RULES and are checked against scanned file paths.
    if (typeof raw.regex !== "string") {
      if (typeof raw.path === "string") {
        const c = convertGoRegex(raw.path);
        new RegExp(c.source, c.flags);
        pathRules.push({ id, path: { source: c.source, flags: c.flags } });
      } else {
        dropped.push(`${id}: has neither regex nor path`);
      }
      continue;
    }
    let regex;
    ...
    // A rule carrying BOTH regex and path is SCOPED: it only applies to files
    // whose path matches (mirrors gitleaks). Without a sourcePath (prompt
    // scanning) the scanner still runs it — recall over precision.
    if (typeof raw.path === "string") {
      const c = convertGoRegex(raw.path);
      new RegExp(c.source, c.flags);
      rule.scopePath = { source: c.source, flags: c.flags };
    }
  ...
  return { rules, pathRules, dropped, loosened, globalAllowlist };
}

function render({ rules, pathRules, globalAllowlist }) {
...
export const PATH_RULES: GenPathRule[] = ${JSON.stringify(pathRules, null, 2)};
